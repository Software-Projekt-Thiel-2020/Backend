...
contract Project is Ownable{
 ...
 //Viele Methoden noch unsicher! Jeder kann sie aufrufen -> welche? (vor allem setter)
  ...
  struct Milestone {
    bytes name;            // muss in hex �bergeben werden
    uint256 donatedAmount;  //bereits gespendet
    uint256 minDonation; //ben�tigte Spenden zum erreichen des Meilensteins, nicht im Pflichtenheft
    uint128 neededVotes;  //   Vom ersteller festgelegt? gibt es ein minimum?
    uint128 minDonToVote; //min. gespendeter Betrag zum w�hlen
 ...
 function payingOut(uint8 milestoneId) onlyOwner public{
   ...
   event Vote(uint8 milestoneId, address donor_add,votePosition vp);
    ...
    //ToDo: �berpr�fen ob der aufrufer der Donor der addresse ist! -> wie?
    // erh�ht den wahl counter des projects(positiv oder negativ).
    //Problem: wenn man in remix value auf 5 ether stellt und 5 zahlt wird counter um 5 erh�ht. Bei wei genau so.
    ...
    //f�r bestimmten milestone spenden
    //spenden auf nicht existierende milestones m�glich
   ...
   //f�gt einen neuen Meilenstein hinzu. Pr�ft ob das Ziel h�her ist als beim letzten (beim ersten nicht)
   // um wie viel muss es gr��er sein?
...
contract Donor is Ownable{
    ...
    //Spender mit bestimmten Projekt verbinden, er muss die Adresse (�ffentlich abrufbar) des Projects eingeben
