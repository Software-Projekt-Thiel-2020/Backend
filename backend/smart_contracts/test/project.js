...
contract('Project', (accounts) => {
    ...
    beforeEach(async () => {
        ...
        uut = await Project.new(accounts[0], accounts[0], 80, web3.utils.fromAscii("TestProject"), 100000, 10, {from: owner});
    });
    afterEach(async () => {
        // nothing to do
    });
    ...
    it('project ctor should revert if partial_payment too big', async () => {
        await truffleAssert.reverts(
            Project.new(accounts[0], accounts[0], 101, web3.utils.fromAscii("TestProject"), 100000, 10, {from: owner})
        );
    });

    it('project ctor should revert if partial_payment is 100%', async () => {
        await truffleAssert.reverts(
            Project.new(accounts[0], accounts[0], 100, web3.utils.fromAscii("TestProject"), 100000, 10, {from: owner})
        );
    });

    it('project ctor should revert if empty project-name', async () => {
        await truffleAssert.reverts(
            Project.new(accounts[0], accounts[0], 80, [], 100000, 10, {from: owner})
        );
    });

    it('project ctor should revert if target amount is 0', async () => {
        await truffleAssert.reverts(
            Project.new(accounts[0], accounts[0], 80, [], 0, 10, {from: owner})
        );
    });
	...
	const test_donation = 100000;
        await uut.register({from: accounts[1]});
        await uut.donate(false, {from: accounts[1], value: test_donation});
 ...
 //assert.equal(milestoneVote,0);
	...
	const test_donation = 100000;
        await uut.register({from: accounts[1]});
        await uut.donate(true, {from: accounts[1], value: test_donation});
	...
	const test_donation = 100000;
        ...
        let expected = (test_target_amount / 100) * 80;
        let rest = test_donation - expected;
    ...
    it('project should not be able to retract after payingOutActiveMilestonePart with positive votes', async () => {
        const test_name = "TestString";
        const test_name2 = "TestString2";
        const test_target_amount = 50000;
        const test_target_amount2 = 75000;
    ...
    it('project should be able to retract after payingOutActiveMilestonePart with negative votes', async () => {
        const test_name = "TestString";
        const test_name2 = "TestString2";
        const test_target_amount = 50000;
        const test_target_amount2 = 75000;
	...
	await truffleAssert.reverts(
        ...
        truffleAssert.eventEmitted(result, 'Retract', (ev) => {
            assert.equal(ev.amount, testDonationAmount, 'event Retract: amount');
            assert.equal(ev.milestoneId, 0, 'event Retract: milestoneId');
            assert.equal(ev.donor, accounts[1], 'event Retract: donor');
            return true;
        });
    ...
    it('project should not be able to vote if donated with wantsToVote = false', async () => {
        const test_name = "TestString";
        const test_name2 = "TestString2";
        const test_target_amount = 50000;
        const test_target_amount2 = 75000;
	...
	const testDonationAmount = 49999;
        ...
        await uut.register({from: accounts[1]});
        await uut.donate(false, {from: accounts[1], value: testDonationAmount});
      //  await uut.vote(0, 1, {from: accounts[1]});
    ...
    it('project should not be able to vote if owner or not registered', async () => {
        const test_name = "TestString";
        const test_name2 = "TestString2";
        const test_target_amount = 50000;
        const test_target_amount2 = 75000;
	...
	await truffleAssert.reverts(
       ...
       await truffleAssert.reverts(
            uut.vote(0,1,{from: accounts[0]})
        );

       await truffleAssert.reverts(
            uut.vote(0,0,{from: accounts[0]})
        );
    });
});
