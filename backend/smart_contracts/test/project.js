...
contract('Project', (accounts) => {
    ...
    beforeEach(async () => {
        ...
        uut = await Project.new(accounts[0], 80, web3.utils.fromAscii("TestProject"), 100000, 10, {from: owner});
    });
    afterEach(async () => {
        // nothing to do
    });
    ...
    it('project ctor should revert if partial_payment too big', async () => {
        await truffleAssert.reverts(
            Project.new(accounts[0], 101, web3.utils.fromAscii("TestProject"), 100000, 10, {from: owner})
        );
    });

    it('project ctor should revert if partial_payment is 100%', async () => {
        await truffleAssert.reverts(
            Project.new(accounts[0], 100, web3.utils.fromAscii("TestProject"), 100000, 10, {from: owner})
        );
    });

    it('project ctor should revert if empty project-name', async () => {
        await truffleAssert.reverts(
            Project.new(accounts[0], 80, [], 100000, 10, {from: owner})
        );
    });

    it('project ctor should revert if target amount is 0', async () => {
        await truffleAssert.reverts(
            Project.new(accounts[0], 80, [], 0, 10, {from: owner})
        );
    });
